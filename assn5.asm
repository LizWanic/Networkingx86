...
printAgain:
; this prints if you have a short file name 
...
shortPrint:
; this prints the backwards file name
	mov     eax, 0x04           ; 4 sys call for write
    mov     ebx, 0x01           ; fd for stdout is 1
...
recieve_initial:
; initialize first three bytes and last byte of recieve_buf to 0 and flag to 0
    mov     byte [recieve_buf], 0
    mov     byte [recieve_buf + 1], 0
    mov     byte [recieve_buf + 2], 0
    mov     byte [recieve_buf + 515], 0
    mov     dword [flag], 0x00
...
; set up for receiving 
    mov     eax, 512                        ; how many bytes to receive
    mov     dword [recieve_buff_len], eax   ; save it as len of buffer
    lea     edx, [recieve_buf + 3]          ; start saving at buf[3]
...
; loop to receive 
    ...
    push    dword [address]             ; pointer to recieve_buf[3]
    push    dword [sock_fd]             ; socket fd 
    mov     ecx, esp                    ; pointer to args
    mov     ebx, 10                     ; sys_recv
    mov     eax, 0x66                   ; sys_socketcall 
...
header_check_loop:
    ...
    je      header_found
    inc     eax 
    cmp     eax, 512                ; check when three bytes left 
...
; put the last three bytes at beginning 
    ...
    mov     dword [recieve_buf], ebx       ; of recieve_buf (plus one null byte)
    ;lea     edx, [recieve_buf + 3]         ; start saving at buf[3]
